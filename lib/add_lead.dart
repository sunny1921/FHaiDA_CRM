...
class _LeadAddPageState extends State<LeadAddPage>
  ...
  static const String _mapboxApiKey =
      'pk.eyJ1Ijoic2FtbWluZ2FzMSIsImEiOiJjbHNicHpjbGEwYnpmMnZsNWRsbzZ1b25qIn0.q03Rgt2P9GjJb6utXNU8LQ';
  ...
  void initState() {
    ...
    _selectedFurnishing = widget.furnishing.toString() ?? 'Unfurnished';
    // _selectedNumberOfBedrooms = widget.bedrooms?.toString() ?? '2';

    _selectedPropertyType =
        widget.propertyType.toString() ?? '2 BHK RESALE FLAT';
    ...
    // _selectedConstructionStatus = 'LEAD RECIEVED';
    // _selectedListedBy = widget.listedBy;
    // _selectedFacing = widget.facing;
    _selectedWantToBuy = 'No';

    _areaSqFtController =
        TextEditingController(text: widget.area) ?? TextEditingController();
    // _descriptionController = TextEditingController(text: widget.description);
    _ownerNameController =
        TextEditingController(text: widget.name) ?? TextEditingController();
    _exactAddressController =
        TextEditingController(text: widget.company) ?? TextEditingController();

    _selectedlandmarks = widget.landmarks ?? ['Kurmannapalem'];
    // _selectedConstructionStatus ;
    _selectedFacing = widget.facing.toString() ?? 'East';
  }

  List<dynamic> _selectedlandmarks = [];
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                _buildMultiSelectDropdownSimpleList(
                  ...
                  initiallySelected: _selectedlandmarks,
                  onChange: (newList) {
                    _selectedlandmarks = newList;
                    // your logic
                  },
                  includeSearch: true,
                ...
                _buildDropdown(
                  'Number of Bedrooms',
                  ['1', '2', '3', '4', '5+'],
                  _selectedNumberOfBedrooms?.toString() ?? '2',
  ...
  void _submitForm() async {
    ...
    specialid1 = specialid1.replaceAll(' ', '');
    int random1 = 1 + Random().nextInt(99);

    var specialid = '${result}${specialid1}${random1}'.toUpperCase();
